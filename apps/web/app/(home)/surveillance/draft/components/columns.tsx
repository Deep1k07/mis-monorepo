...
export type DraftSurveillanceDef = {
  _id: string;
  entity_id?: string;
  entity_name?: string;
...
const scopeBadge = (status: string) => {
...
export const createDraftSurveillanceColumns = (
  onView: (row: DraftSurveillanceDef) => void,
): ColumnDef<DraftSurveillanceDef>[] => [
  {
    id: "entity_id",
    header: "Entity ID",
  ...
  },
  {
    ...
    header: "Scope Status",
    cell: ({ row }) => scopeBadge(row.original.scopeStatus ?? "pending"),
  },
  {
    accessorKey: "survApplied",
    header: "Applied On",
    cell: ({ row }) => {
      const value = row.original.survApplied;
