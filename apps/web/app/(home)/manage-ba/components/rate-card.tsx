...
export function CbRateCardSection({
  ...
  const toggleStandard = (cbIndex: number, standard: any) => {
    ...
    const alreadyIndex = existing.findIndex((s) => s.code === standard.mssCode);
    ...
    <div className="space-y-4">
      ...
      {cbEntries.map((cb, cbIndex) => {
        const selectedCab = cabs.find(
          (c: any) => c.cabCode === cb.cabCode && c.cbCode === cb.cbCode,
        );
            ...
            <div>
              ...
              >
                <SelectTrigger className="mt-1">
                  <SelectValue
                    placeholder={loadingCabs ? "Loading..." : "Select CAB"}
                  >
                    {(value: string | null) => {
                      if (!value)
