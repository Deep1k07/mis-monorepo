...
export const createBaColumns = (
  onEdit: (ba: BaDef) => void,
    ...
    },
    {
      ...
      header: "Username",
      cell: ({ row }) => (
        <span className="font-medium truncate">{row.getValue("username")}</span>
      ),
    },
    {
      accessorKey: "email",
    ...
    },
    {
      ...
      header: "Contact Name",
      cell: ({ row }) => (
        <span className="block max-w-[200px] truncate">{row.original.cab?.contact_name || "-"}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span
            className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium capitalize ring-1 ring-inset ${status === "active"
