...
export class AuthService {
    ...
    async login(body: LoginDto) {
        ...
        if (!isPasswordCorrect) throw new NotFoundException('Invalid Credentials :(');
        if (user?.status === 'inactive' || user?.status === 'suspended' || user?.status === 'rejected') {
            throw new BadRequestException('Your account has been deactivated, please contact with administrator.');
        }

        // Two-Factor Authentication (2FA)
        if (user?.twoFA?.enabled) {
            if (!body.code) {
                return { success: false, twoFa: true, msg: "Two-factor authentication is enabled. Please verify your code." };
            }
